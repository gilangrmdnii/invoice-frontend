...
import type { ApiResponse, BudgetRequest, CreateBudgetRequest } from '../types';
...
export const budgetRequestApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    approveBudgetRequest: builder.mutation<ApiResponse<BudgetRequest>, number>({
      query: (id) => ({
    ...
    rejectBudgetRequest: builder.mutation<ApiResponse<BudgetRequest>, number>({
      query: (id) => ({
